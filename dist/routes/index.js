"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const express_1 = require("express");
const auth_routes_1 = __importDefault(require("./auth.routes"));
const project_routes_1 = __importDefault(require("./project.routes"));
const task_routes_1 = __importDefault(require("./task.routes"));
const invite_routes_1 = __importDefault(require("./invite.routes"));
const organization_routes_1 = __importDefault(require("./organization.routes"));
const user_organizations_routes_1 = __importDefault(require("./user-organizations.routes"));
const workflow_routes_1 = __importDefault(require("./workflow.routes"));
const channel_routes_1 = __importDefault(require("./channel.routes"));
const friend_routes_1 = __importDefault(require("./friend.routes"));
const worklog_routes_1 = __importDefault(require("./worklog.routes"));
const team_role_routes_1 = __importDefault(require("./team-role.routes"));
const meeting_routes_1 = __importDefault(require("./meeting.routes"));
const mom_routes_1 = __importDefault(require("./mom.routes"));
const public_meeting_routes_1 = __importDefault(require("./public-meeting.routes"));
const router = (0, express_1.Router)();
router.use("/auth", auth_routes_1.default);
router.use("/projects", project_routes_1.default);
router.use("/tasks", task_routes_1.default);
router.use("/invites", invite_routes_1.default);
router.use("/organizations", organization_routes_1.default);
router.use(user_organizations_routes_1.default);
router.use("/workflows", workflow_routes_1.default);
router.use("/channels", channel_routes_1.default);
router.use("/friends", friend_routes_1.default);
router.use("/worklogs", worklog_routes_1.default);
router.use(team_role_routes_1.default);
router.use("/meetings", meeting_routes_1.default);
router.use("/mom", mom_routes_1.default);
router.use("/public", public_meeting_routes_1.default);
exports.default = router;
